...
import {createTruck, updateTruck} from "../HTTPS/truckApi";
import {useParams} from "react-router-dom";

const AddEditTruck = observer(({show, onHide, minYear, maxYear, truck, type}) => {
    const {types} = useContext(Context)
    const {id} = useParams();
    const dispatch = useNotification();

    const [formValues, setFormValues] = useState(truck);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if(error)
                return
            if(type == 'Create') {
                await createTruck(formValues)
            }
            if(type == 'Update') {
                await updateTruck(id, formValues)
            }
    ...
    return (
        <Modal show={show} onHide={onHide} centered role="dialog">
            <Modal.Header>
                <h3 className="text-center">{type} truck</h3>
            </Modal.Header>
